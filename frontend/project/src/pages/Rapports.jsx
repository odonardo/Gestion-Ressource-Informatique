...
// // // src/pages/Rapports.jsx - VERSION FINALE SANS API NON DISPONIBLES
// // import React, { useState, useEffect } from 'react';
// // import { 
// //   FileText, Download, Filter, TrendingUp, 
// //   RefreshCw, Monitor, Package, AlertTriangle, 
// //   Users, Database, Search, Bell, User, Wrench
// // } from 'lucide-react';
// // // Import seulement des API disponibles
// // import { 
// //   materielsAPI, incidentsAPI, reparationsAPI, 
// //   alertesAPI, fournisseursAPI, usersAPI
// // } from '../services/api';
// // import jsPDF from 'jspdf';
// // import 'jspdf-autotable';
// // import autoTable from 'jspdf-autotable';
...
// //     doc.text(`• En panne: ${statsGenerales.materiels_en_panne || 0}`, 160, yPos);
// //     yPos += 7;
// //     doc.text(`• Incidents ouverts: ${statsGenerales.incidents_ouverts || 0}`, 20, yPos);
// //     doc.text(`• Alertes actives: ${statsGenerales.alertes_actives || 0}`, 90, yPos);
// //     doc.text(`• Coût total réparations: ${formatCurrency(statsGenerales.cout_total_reparations || 0)}`, 160, yPos);
// //     yPos += 7;
// //     doc.text(`• Utilisateurs: ${statsGenerales.total_utilisateurs || 0}`, 20, yPos);
// //     doc.text(`• Fournisseurs: ${statsGenerales.total_fournisseurs || 0}`, 90, yPos);
...
// //     const tables = [
// //       { key: 'materiels', title: '2. INVENTAIRE DES MATÉRIELS' },
// //       { key: 'incidents', title: '3. RAPPORT DES INCIDENTS' },
// //       { key: 'reparations', title: '4. RAPPORT DES RÉPARATIONS' },
// //       { key: 'alertes', title: '5. RAPPORT DES ALERTES' },
// //       { key: 'fournisseurs', title: '6. LISTE DES FOURNISSEURS' },
// //       { key: 'utilisateurs', title: '7. PROFIL UTILISATEURS' }
// //     ];
...
// //       doc.setTextColor(150, 150, 150);
// //       doc.text(
// //         `Système de Gestion des Ressources materiels Informatiques - DREN Antsimo Andrefana - Page ${i}/${pageCount}`,
// //         20,
// //         doc.internal.pageSize.height - 10
// //       );
// //     }
...
// //             <span className="badge badge-warning">Incidents: {data.incidents.length}</span>
// //             <span className="badge badge-error">Alertes: {data.alertes.length}</span>
// //             <span className="badge badge-accent">Réparations: {data.reparations.length}</span>
// //             <span className="badge badge-neutral">Utilisateurs: {data.utilisateurs.length}</span>
// //           </div>
// //         </div>
// //         <div className="flex gap-2">
// //           <button onClick={loadData} className="btn btn-outline btn-sm">
...
// //             {(currentTable === 'materiels' || currentTable === 'incidents' || currentTable === 'alertes') && (
// //               <div className="form-control">
// //                 <label className="label">
// //                   <span className="label-text">
// //                     {currentTable === 'alertes' ? 'Sévérité' : 'État'}
...
// //                 <select
// //                   className="select select-bordered bg-base-100"
// //                   value={currentTable === 'alertes' ? filters.type : filters.etat}
// //                   onChange={(e) => {
// //                     if (currentTable === 'alertes') {
// //                       setFilters({...filters, type: e.target.value});
// //                     } else {
// //                       setFilters({...filters, etat: e.target.value});
// //                     }
...
// //             <button
// //               onClick={() => setFilters({
// //                 dateDebut: '', dateFin: '', service: '', etat: '', searchTerm: ''
// //               })}
// //               className="btn btn-ghost btn-sm"
// //             >
// //               Réinitialiser les filtres
...
// // src/pages/Rapports.jsx - VERSION CORRIGÉE AVEC COULEURS
// import React, { useState, useEffect } from 'react';
// import { 
//   FileText, Download, Filter, TrendingUp, 
//   RefreshCw, Monitor, Package, AlertTriangle, 
...
//   // Couleurs pour chaque type de tableau
//   const tableColors = {
//     materiels: { 
//       badge: 'badge-primary',
//       stat: 'text-primary',
...
//       badge: 'badge-warning',
//       stat: 'text-warning',
//       pdf: [245, 158, 11] // Orange
//     },
//     reparations: { 
//       badge: 'badge-success',
//       stat: 'text-success',
//       pdf: [34, 197, 90] // Vert (success)
//     },
//     alertes: { 
//       badge: 'badge-error',
//       stat: 'text-error',
...
//         { header: 'Prénom', accessor: 'first_name', testField: 'first_name' },
//         { header: 'Nom', accessor: 'last_name', testField: 'last_name' },
//         { header: 'Département', accessor: 'departement', testField: 'departement' },
//         { header: 'Rôle', accessor: 'role', testField: 'role' },
//         { header: 'Statut', accessor: 'is_active', testField: 'is_active', formatter: formatActif },
//         { header: 'Téléphone', accessor: 'telephone', testField: 'telephone' },
//         { header: 'Date Inscription', accessor: 'date_joined', testField: 'date_joined', formatter: formatDate }
...
//     // Informations de filtrage avec couleurs
//     doc.setFontSize(10);
//     let infoY = yPos;
...
//     }
//     if (filters.etat) {
//       doc.text(`État: ${filters.etat}`, 20, infoY);
//       infoY += 6;
//     }
//     if (filters.searchTerm) {
//       doc.text(`Recherche: ${filters.searchTerm}`, 20, infoY);
//       infoY += 6;
//     }
    
//     yPos = infoY + 5;
    
//     // Statistiques avec couleur correspondante
//     doc.setFontSize(11);
//     doc.setTextColor(0, 0, 0);
//     const color = tableColors[currentTable]?.pdf || [59, 130, 246];
...
//       styles: { fontSize: 8, cellPadding: 3 },
//       headStyles: { 
//         fillColor: tableColors[currentTable]?.pdf || [59, 130, 246],
...
//     }
//     if (filters.etat) {
//       metadata.push([`État: ${filters.etat}`]);
//     }
//     if (filters.searchTerm) {
...
//           styles: { fontSize: 7, cellPadding: 2 },
//           headStyles: { 
//             fillColor: tableColors[table.key]?.pdf || [59, 130, 246],
//             textColor: [255, 255, 255]
//           },
//           margin: { left: 20, right: 20 },
//           pageBreak: 'auto'
...
//           </p>
//           <div className="flex flex-wrap gap-2 mt-2 text-sm">
//             <span className={`badge ${tableColors.fournisseurs.badge}`}>Fournisseurs: {data.fournisseurs.length}</span>
//             <span className={`badge ${tableColors.materiels.badge}`}>Matériels: {data.materiels.length}</span>
//             <span className={`badge ${tableColors.incidents.badge}`}>Incidents: {data.incidents.length}</span>
//             <span className={`badge ${tableColors.alertes.badge}`}>Alertes: {data.alertes.length}</span>
//             <span className={`badge ${tableColors.reparations.badge}`}>Réparations: {data.reparations.length}</span>
//             <span className={`badge ${tableColors.utilisateurs.badge}`}>Utilisateurs: {data.utilisateurs.length}</span>
//           </div>
//         </div>
//         <div className="flex gap-2">
//           <button onClick={loadData} className="btn btn-outline btn-sm">
...
//             <h2 className="card-title text-base-content">
//               <Filter className="h-5 w-5" />
//               Filtres pour: <span className={tableColors[currentTable]?.stat || 'text-primary'}>{tableTypes.find(t => t.key === currentTable)?.label}</span>
//               <span className="text-sm font-normal ml-2">
//                 ({getFilteredData().length} éléments, {dynamicColumns[currentTable]?.length || 0} colonnes)
//               </span>
//             </h2>
//             <div className="flex gap-2">
//               <div className="dropdown dropdown-end">
//                 <div tabIndex={0} role="button" className={`btn ${currentTable === 'materiels' ? 'btn-primary' : currentTable === 'incidents' ? 'btn-warning' : currentTable === 'reparations' ? 'btn-accent' : currentTable === 'alertes' ? 'btn-error' : currentTable === 'fournisseurs' ? 'btn-info' : 'btn-neutral'} btn-sm`}>
//                   <Download className="h-4 w-4 mr-2" />
//                   Exporter
//                 </div>
//                 <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
...
//       {/* Tableau principal - SANS COULEURS DANS LES CELLULES */}
//       <div className="card bg-base-100 shadow-xl border border-base-300">
//         <div className="card-body p-0">
//           <div className="overflow-x-auto">
//             <table className="table table-zebra w-full">
...
//                         // PAS DE COULEURS DANS LES CELLULES - Comme réparations
//                         return (
//                           <td key={colIndex}>
//                             {formattedValue}
//                           </td>
...
//       {/* Statistiques rapides avec couleurs */}
//       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
//         <div className={`stat bg-base-200 rounded-lg p-4 border-l-4 ${tableColors.materiels.badge.replace('badge-', 'border-')}`}>
//           <div className="stat-figure">
//             <Monitor className={`h-8 w-8 ${tableColors.materiels.stat}`} />
//           </div>
//           <div className={`stat-title ${tableColors.materiels.stat}`}>Matériels</div>
//           <div className="stat-value">{statsGenerales.total_materiels || 0}</div>
//           <div className="stat-desc">
//             {statsGenerales.materiels_fonctionnels || 0} fonctionnels • {statsGenerales.materiels_en_panne || 0} en panne
//           </div>
...
//         <div className={`stat bg-base-200 rounded-lg p-4 border-l-4 ${tableColors.incidents.badge.replace('badge-', 'border-')}`}>
//           <div className="stat-figure">
//             <AlertTriangle className={`h-8 w-8 ${tableColors.incidents.stat}`} />
//           </div>
//           <div className={`stat-title ${tableColors.incidents.stat}`}>Incidents</div>
//           <div className="stat-value">{statsGenerales.incidents_ouverts || 0}</div>
//           <div className="stat-desc">
//             Ouverts • Total: {statsGenerales.incidents_total || 0}
//           </div>
...
//         <div className={`stat bg-base-200 rounded-lg p-4 border-l-4 ${tableColors.reparations.badge.replace('badge-', 'border-')}`}>
//           <div className="stat-figure">
//             <Wrench className={`h-8 w-8 ${tableColors.reparations.stat}`} />
//           </div>
//           <div className={`stat-title ${tableColors.reparations.stat}`}>Réparations</div>
//           <div className="stat-value">{statsGenerales.reparations_total || 0}</div>
//           <div className="stat-desc">
//             Coût: {formatCurrency(statsGenerales.cout_total_reparations || 0)}
//           </div>
...
//       {/* Deuxième ligne de statistiques */}
//       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
//         <div className={`stat bg-base-200 rounded-lg p-4 border-l-4 ${tableColors.alertes.badge.replace('badge-', 'border-')}`}>
//           <div className="stat-figure">
//             <Bell className={`h-8 w-8 ${tableColors.alertes.stat}`} />
//           </div>
//           <div className={`stat-title ${tableColors.alertes.stat}`}>Alertes</div>
//           <div className="stat-value">{statsGenerales.alertes_actives || 0}</div>
//           <div className="stat-desc">
//             Actives • Total: {statsGenerales.alertes_total || 0}
//           </div>
...
//         <div className={`stat bg-base-200 rounded-lg p-4 border-l-4 ${tableColors.fournisseurs.badge.replace('badge-', 'border-')}`}>
//           <div className="stat-figure">
//             <Users className={`h-8 w-8 ${tableColors.fournisseurs.stat}`} />
//           </div>
//           <div className={`stat-title ${tableColors.fournisseurs.stat}`}>Fournisseurs</div>
//           <div className="stat-value">{statsGenerales.total_fournisseurs || 0}</div>
//           <div className="stat-desc">
//             Partenaires et contacts
//           </div>
...
//         <div className={`stat bg-base-200 rounded-lg p-4 border-l-4 ${tableColors.utilisateurs.badge.replace('badge-', 'border-')}`}>
//           <div className="stat-figure">
//             <User className={`h-8 w-8 ${tableColors.utilisateurs.stat}`} />
//           </div>
//           <div className={`stat-title ${tableColors.utilisateurs.stat}`}>Utilisateurs</div>
//           <div className="stat-value">{statsGenerales.total_utilisateurs || 0}</div>
//           <div className="stat-desc">
//             Utilisateurs du système
//           </div>
...
// src/pages/Rapports.jsx - VERSION AVEC AUTO-LOGGER CORRIGÉE
import React, { useState, useEffect } from 'react';
import { 
  FileText, Download, Filter, TrendingUp, 
  RefreshCw, Monitor, Package, AlertTriangle, 
...
const useAutoLogger = () => {
  ...
  const logExport = (format, module, dataCount, filters = {}) => {
  ...
  const logReportGeneration = (reportType, format, dataCount) => {
    logAction('GÉNÉRATION', 'Rapports', `${reportType} en ${format} (${dataCount} éléments)`, {
  ...
  const logDataLoad = (tableCounts) => {
    logAction('CHARGEMENT', 'Rapports', `Chargement des données: ${JSON.stringify(tableCounts)}`, {
      counts: tableCounts,
      type: 'load'
    });
  };
  
  const logFilter = (module, filterType, value, resultsCount) => {
    logAction('FILTRAGE', 'Rapports', `Filtre ${filterType}: ${value} (${resultsCount} résultats)`, {
      module: module,
  ...
  const logSearch = (module, searchTerm, resultsCount) => {
    logAction('RECHERCHE', 'Rapports', `Recherche "${searchTerm}" (${resultsCount} résultats)`, {
      module: module,
  ...
  return {
    ...
    // Fonctions spécifiques pour les rapports
    logTableChange: (fromTable, toTable) =>
      logAction('NAVIGATION', 'Rapports', `Changement de tableau: ${fromTable} → ${toTable}`, {
        from: fromTable,
        to: toTable,
        type: 'table_change'
      }),
    
    logFullReportExport: (format, totalCount) =>
      logReportGeneration('Rapport complet', format, totalCount),
    
    logTableExport: (table, format, count, filters) =>
      logExport(format, table, count, filters),
    
    logReportRefresh: () =>
      logAction('RAFRAÎCHISSEMENT', 'Rapports', 'Rafraîchissement des données', {
        type: 'refresh',
        timestamp: new Date().toISOString()
      })
  };
...
const Rapports = () => {
  ...
  const [currentTable, setCurrentTable] = useState('materiels');
  const [prevTable, setPrevTable] = useState('materiels');
  const [statsGenerales, setStatsGenerales] = useState({});
  const [dynamicColumns, setDynamicColumns] = useState({});
  const [logoBase64, setLogoBase64] = useState(null);
  ...
  const loadData = async () => {
    try {
      ...
      // 🔥 AUTO-LOGGER: Chargement des données
      const tableCounts = {
        materiels: results.materiels.length,
        incidents: results.incidents.length,
        reparations: results.reparations.length,
        alertes: results.alertes.length,
        fournisseurs: results.fournisseurs.length,
        utilisateurs: results.utilisateurs.length
      };
      autoLogger.logDataLoad(tableCounts);
      
      showNotification('success', `Données chargées: ${Object.values(tableCounts).reduce((a, b) => a + b, 0)} éléments`, 'Chargement réussi');
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      showNotification('info', 'Chargement des données de rapports...', 'Bienvenue');
    }, 500);
  }, []);
  ...
  const generatePDFReport = () => {
    ...
    // 🔥 AUTO-LOGGER: Export PDF
    autoLogger.logTableExport(currentTable, 'PDF', filteredData.length, {
    ...
    showNotification('success', `Export PDF réussi: ${filteredData.length} éléments`, 'Export PDF');
  ...
  const generateExcelReport = () => {
    ...
    // 🔥 AUTO-LOGGER: Export Excel
    autoLogger.logTableExport(currentTable, 'Excel', filteredData.length, {
      searchTerm: filters.searchTerm,
      service: filters.service,
      etat: filters.etat,
      dateDebut: filters.dateDebut,
    ...
    showNotification('success', `Export Excel réussi: ${filteredData.length} éléments`, 'Export Excel');
  ...
  const generateCSVReport = () => {
    ...
    // 🔥 AUTO-LOGGER: Export CSV
    autoLogger.logTableExport(currentTable, 'CSV', filteredData.length, {
      searchTerm: filters.searchTerm,
      service: filters.service,
      etat: filters.etat,
      dateDebut: filters.dateDebut,
    ...
    showNotification('success', `Export CSV réussi: ${filteredData.length} éléments`, 'Export CSV');
  ...
  const generateRapportCompletPDF = () => {
    const totalCount = Object.values(data).reduce((sum, table) => sum + table.length, 0);
    
    if (totalCount === 0) {
      showNotification('error', 'Aucune donnée à exporter pour le rapport complet !', 'Erreur');
      return;
    }
    
    // 🔥 AUTO-LOGGER: Export rapport complet PDF
    autoLogger.logFullReportExport('PDF', totalCount);
    
    showNotification('info', `Génération du rapport complet PDF (${totalCount} éléments)...`, 'Export en cours');
    ...
    showNotification('success', 'Rapport complet PDF généré avec succès', 'Export réussi');
  };
  ...
  const handleTableChange = (newTable) => {
    ...
    // 🔥 AUTO-LOGGER: Changement de tableau
    autoLogger.logTableChange(currentTable, newTable);
  };

  const handleRefresh = () => {
    // 🔥 AUTO-LOGGER: Rafraîchissement des données
    autoLogger.logReportRefresh();
    loadData();
  };
  ...
  const handleResetFilters = () => {
    ...
    autoLogger.logAction('RÉINITIALISATION', 'Rapports', 'Filtres réinitialisés', {
      type: 'reset_filters',
      table: currentTable
    });
  };
    ...
    <div className="p-6 bg-base-100 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button 
            onClick={generateRapportCompletPDF} 
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value.trim()) {
                    const filteredData = [...data[currentTable]].filter(item => {
                      return Object.values(item).some(value => 
                        value && value.toString().toLowerCase().includes(e.target.value.toLowerCase())
                      );
                    });
                    autoLogger.logSearch(currentTable, e.target.value, filteredData.length);
                  }
                }}
              />
            </div>
              ...
              <div className="form-control">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value) {
                      const filteredData = [...data[currentTable]].filter(m => 
                        m.service_attribue === e.target.value || 
                        m.service === e.target.value ||
                        m.departement === e.target.value
                      );
                      autoLogger.logFilter(currentTable, 'service', e.target.value, filteredData.length);
                    }
                  }}
                >
                  <option value="">Tous les services</option>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (currentTable === 'alertes') {
                      ...
                      // 🔥 AUTO-LOGGER: Filtre par type/sévérité
                      if (value) {
                        const filteredData = [...data[currentTable]].filter(a => 
                          a.statut?.toLowerCase() === value.toLowerCase() ||
                          a.severite?.toLowerCase() === value.toLowerCase()
                        );
                        autoLogger.logFilter(currentTable, 'type/sévérité', value, filteredData.length);
                      }
                    } else {
                      ...
                      if (value) {
                        const filteredData = [...data[currentTable]].filter(item => 
                          item.etat?.toLowerCase() === value.toLowerCase() ||
                          item.statut?.toLowerCase() === value.toLowerCase()
                        );
                        autoLogger.logFilter(currentTable, 'état', value, filteredData.length);
                      }
                    }
                  }}
                >
                ...
                <div className="form-control">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      // 🔥 AUTO-LOGGER: Filtre par date début
                      if (e.target.value) {
                        const filteredData = [...data[currentTable]].filter(item => {
                          const itemDate = new Date(item.date_signalement || item.date_creation || item.date_debut || item.date_alerte || item.created_at);
                          return itemDate >= new Date(e.target.value);
                        });
                        autoLogger.logFilter(currentTable, 'date début', e.target.value, filteredData.length);
                      }
                    }}
                  />
                </div>
                ...
                <div className="form-control">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      // 🔥 AUTO-LOGGER: Filtre par date fin
                      if (e.target.value) {
                        const filteredData = [...data[currentTable]].filter(item => {
                          const itemDate = new Date(item.date_signalement || item.date_creation || item.date_debut || item.date_alerte || item.created_at);
                          return itemDate <= new Date(e.target.value);
                        });
                        autoLogger.logFilter(currentTable, 'date fin', e.target.value, filteredData.length);
                      }
                    }}
                  />
                </div>
