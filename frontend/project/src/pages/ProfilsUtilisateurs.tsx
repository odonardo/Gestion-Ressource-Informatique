...
const ProfilsUtilisateurs: React.FC = () => {
  ...
  const calculerStatistiques = (data: ProfilUtilisateur[]) => {
    ...
    setStatistiques(prev => ({
      ...prev,
      total: data.length,
      actifs: data.filter(p => p.user?.is_active !== false).length,
      administrateurs,
      techniciens,
      parDepartement,
      parRole
  ...
  const filterProfils = () => {
    ...
    if (searchTerm) {
      filtered = safeFilter(filtered, profil => {
        const userName = profil.user?.username || '';
        const userFirstName = profil.user?.first_name || '';
        const userLastName = profil.user?.last_name || '';
        const userEmail = profil.user?.email || '';
  ...
  const handleExport = () => {
    try {
      const dataToExport = filteredProfils.map(p => ({
        'Username': p.user?.username || 'Non spécifié',
        'Email': p.user?.email || 'Non spécifié',
        'Prénom': p.user?.first_name || '',
        ...
        'Statut': p.user?.is_active !== false ? 'Actif' : 'Inactif'
      }));
              ...
              <tbody>
                {safeArray(filteredProfils).map((profil) => (
                  <tr key={profil.id} className="hover">
                        ...
                        <div className="avatar placeholder">
                          <div className="bg-neutral text-neutral-content rounded-full w-10">
                            <span className="text-sm">
                              {profil.user?.username?.[0]?.toUpperCase() || 'U'}
                            </span>
                          </div>
                        </div>
                        <div>
                          <div className="font-semibold">
                            @{profil.user?.username || 'Non spécifié'}
                          </div>
                    ...
                    <td>
                      <div className={`badge ${getStatusBadge(profil.user?.is_active)} badge-lg`}>
                        {getStatusText(profil.user?.is_active)}
                      </div>
                    </td>
                    <td>
                      {profil.telephone ? (
                        ...
                        <button
                          ...
                          onClick={() => {
                            showMessage('info', 
                              ...
                              Nom complet: ${profil.user?.first_name} ${profil.user?.last_name}
